...
// Dynamic WooCommerce Store API Proxy Route
// Handles server-to-server relay to cleanly avoid browser CORS restrictions
export const ALL: APIRoute = async ({ params, request }) => {
  const pathSuffix = params.path ? `/${params.path}` : '';
  
  // Extract WordPress host from local configuration
  const graphqlUrl = import.meta.env.PUBLIC_GRAPHQL_URL || 'http://localhost:8080/graphql';
  const baseUrl = graphqlUrl.replace(/\/graphql$/, '');
  const targetUrl = `${baseUrl}/wp-json/wc/store/v1/cart${pathSuffix}`;
  ...
  try {
    ...
    // Forward any fresh Nonce token returned by WordPress back to the client
    ...
    if (remoteNonce) {
      console.log(`[Proxy] Captured Nonce from WP: ${remoteNonce}`);
    } else {
      console.log(`[Proxy] No Nonce found in WP response headers. Status: ${remoteResponse.status}`);
    }

    const responseHeaders: Record<string, string> = {
      'Content-Type': 'application/json',
      'Access-Control-Expose-Headers': 'X-WC-Store-API-Nonce, Nonce, Cart-Token', // Critical for browser visibility
    };
